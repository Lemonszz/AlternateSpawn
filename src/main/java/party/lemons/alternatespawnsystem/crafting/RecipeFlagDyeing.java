package party.lemons.alternatespawnsystem.crafting;
...
public class RecipeFlagDyeing extends net.minecraftforge.registries.IForgeRegistryEntry.Impl<IRecipe> implements IRecipe
{
	...
	public boolean matches(InventoryCrafting inv, World worldIn)
	{
		...
		for (int i = 0; i < inv.getSizeInventory(); ++i)
		{
			...
			{
				if (itemstack1.getItem() instanceof ItemBlockFlag)
				{
	...
	public ItemStack getCraftingResult(InventoryCrafting inv)
	{
		...
		for (int k = 0; k < inv.getSizeInventory(); ++k)
		{
			...
			if (!itemstack1.isEmpty())
			{
				if (itemstack1.getItem() instanceof ItemBlockFlag)
				{
					itemblock = (ItemBlockFlag)itemstack1.getItem();

					itemstack = itemstack1.copy();
					itemstack.setCount(1);

					if (itemblock.hasColor(itemstack1))
					{
						int l = itemblock.getColor(itemstack);
		...
		else
		{
			...
			int k2 = (i1 << 8) + j1;
			k2 = (k2 << 8) + k1;
			itemblock.setColor(itemstack, k2);
			return itemstack;
		}
	}
